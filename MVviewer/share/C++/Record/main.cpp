...
int main()
{
    ...
    int64_t nWidth;
    CIntNode nodeWidth = CIntNode(cameraSptr, "Width");
    if (nodeWidth.isValid() && nodeWidth.getValue(nWidth))
    {
    ...
    int64_t nHeight;
    CIntNode nodeHeight = CIntNode(cameraSptr, "Height");
    if (nodeHeight.isValid() && nodeHeight.getValue(nHeight))
    {
	...
	// 打开录像句柄
    // open the record g_hHandle
    if (RECORD_SUCCESS == openRecord(&sRecordParam, &g_hHandle))
    {
		// 线程开始取图
		...
		// 关闭录像句柄
        // close the record g_hHandle
        closeRecord(g_hHandle);
	}
	else
    {
