...
static int32_t setSoftTriggerConf(GENICAM_AcquisitionControl *pAcquisitionCtrl)
{
	int32_t nRet;
...
static int32_t executeTriggerSoftware(GENICAM_AcquisitionControl *pAcquisitionCtrl)
{
	...
	if(isTriggerSoftwareSuccess != 0)
	{
		...
		// Attention: should release cmdNode internal object before return
		cmdNode.release(&cmdNode);
		return -1;
	}

	// 注意：需要释放cmdNode内部对象内存
	// Attention: should release cmdNode internal object at the end of use
	cmdNode.release(&cmdNode);
